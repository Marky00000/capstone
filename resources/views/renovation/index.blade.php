...
@section('title', 'Renovation Services')
@section('content')
                    ...
                    @foreach($services as $service)
                        <tr>
                            ...
                            <td style="text-align: center;">
                                @if($service->design)
                                    <img src="{{ asset('storage/' . $service->design) }}" alt="{{ $service->name }}" style="width: 100px; height: auto; border: 1px solid #ccc; display: block; margin: 0 auto;">
                                @else
                                    No Image
                                @endif
                            </td>
                            ...
                            <td>
                                <div style="display: flex; justify-content: space-evenly; align-items: center; gap: 10px; padding: 8px 0;">
                                    ...
                                    <a href="{{ route('renovation-services.edit', $service->id) }}" class="btn btn-sm" style="background-color: transparent; border: none; color: #007bff; outline: none;" data-toggle="tooltip" title="Edit Service">
...
<div class="modal fade" id="archiveConfirmModal" tabindex="-1" aria-labelledby="archiveConfirmModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p id="archiveConfirmMessage">Are you sure you want to archive <strong id="archiveServiceName"></strong>?</p>
...
<script>
    ...
    $('#archiveConfirmModal').on('show.bs.modal', function (event) {
        ...
        modal.find('#archiveForm').attr('action', '{{ route('renovation-services.archive', ':id') }}'.replace(':id', serviceId));
    });
