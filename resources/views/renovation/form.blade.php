...
<h4>Renovation Services</h4>
    ...
    <div class="container mt-5">
        <div class="card shadow-lg">
            <div class="card-header"
                style="background-image: url('{{ asset('renovation.jpg') }}'); height: 180px; background-size: cover; background-position: center;">
            </div>
            ...
            <div class="card-body p-4">
                ...
                <form id="renovationServiceForm"
                    action="{{ isset($service) ? route('renovation-services.update', $service->id) : route('renovation-services.store') }}"
                    ...
                    <div class="form-group mb-4">
                        ...
                        <select class="form-select @error('complexity') is-invalid @enderror" id="complexity"
                    ...
                    <div class="form-group mb-4">
                        ...
                        <select class="form-select @error('type') is-invalid @enderror" id="type" name="type"
                            ...
                            <option value="landscaping"
                                {{ old('type', isset($service) ? $service->type : '') == 'landscaping' ? 'selected' : '' }}>
                                Landscaping</option>
                            <option value="swimmingpool"
                                {{ old('type', isset($service) ? $service->type : '') == 'swimmingpool' ? 'selected' : '' }}>
                    ...
                    <div class="form-group mb-4">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                            rows="3" required>{{ old('description', isset($service) ? $service->description : '') }}</textarea>
                        @error('description')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
    ...
    <script>
        document.getElementById('renovationServiceForm').addEventListener('submit', function(event) {
            ...
            if (designInput.files.length === 0 && !
                {{ isset($service) ? 'true' : 'false' }}) { // Check if service is not being edited
