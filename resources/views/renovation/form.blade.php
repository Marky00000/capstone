...
<div class="container mt-5">
    <div class="card shadow-lg">
        <div class="card-header" style="background-image: url('{{ asset('renovation.jpg') }}'); height: 180px; background-size: cover; background-position: center;">
        </div>
            ...
            <form id="renovationServiceForm" action="{{ isset($service) ? route('renovation-services.update', $service->id) : route('renovation-services.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group mb-4">
                    ...
                    <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                        <option value="landscaping" {{ old('type', isset($service) ? $service->type : '') == 'landscaping' ? 'selected' : '' }}>Landscaping</option>
                        <option value="swimmingpool" {{ old('type', isset($service) ? $service->type : '') == 'swimmingpool' ? 'selected' : '' }}>Swimming Pool</option>
                    </select>
                    @error('type')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                ...
                <div class="form-group mb-4">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="3">{{ old('description', isset($service) ? $service->description : '') }}</textarea>
                    @error('description')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <a href="{{ route('renovation') }}" class="btn btn-outline-secondary rounded-pill px-4 py-2">Cancel</a>
                </div>
            </form>
        </div>
    </div>
