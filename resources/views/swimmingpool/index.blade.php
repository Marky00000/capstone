...
<div class="card">
    <div class="card-header stylish-header">
        <div class="card-tools">
            <a href="{{ route('swimmingpool-services.create') }}" class="btn btn-primary">Add New</a>
        </div>
    </div>
    <div class="card-body">
        @if(session('success'))
                        ...
                        <tr>
                            ...
                            <td>
                                <div style="display: flex; justify-content: space-evenly; align-items: center; gap: 10px; padding: 8px 0;">
                                    ...
                                    <a href="{{ route('swimmingpool-services.edit', $service->id) }}" class="btn btn-sm" style="background-color: transparent; border: none; color: #007bff; outline: none;" data-toggle="tooltip" title="Edit Service">
...
<script>
    ...
    $('#archiveConfirmModal').on('show.bs.modal', function (event) {
        ...
        modal.find('#archiveForm').attr('action', '{{ route('swimmingpool-services.archive', ':id') }}'.replace(':id', serviceId));
    });
