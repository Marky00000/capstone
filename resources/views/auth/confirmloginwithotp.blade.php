        ...
        <form id="otp-form" method="POST" action="{{ route('confirm.login.with.otp.post') }}">
            ...
            <div class="form-group mb-3 text-center">
                ...
                <div class="d-flex justify-content-center mt-3">
                    @for ($i = 0; $i < 6; $i++)
                        <input id="otp{{ $i }}" type="text" class="form-control otp-input mx-1 @error('otp') is-invalid @enderror" name="otp[]" maxlength="1" required>
                    @endfor
                </div>
                @error('otp')
            ...
            <div class="form-group mb-0 text-center">
                <button type="submit" class="btn btn-custom btn-block py-2 font-weight-bold">
                    {{ __('Verify OTP') }}
                </button>
            </div>

            <div id="send-another-link" class="text-center mb-3">
                <button id="send-another-otp" class="btn btn-link text-decoration-none">Send another OTP</button>
            </div>
            ...
            countdownInterval = setInterval(() => {
                ...
                if (countdown <= 0) {
                    ...
                    fetch('{{ route("set.otp.null") }}', {
                        ...
                        body: JSON.stringify({ email: "{{ request()->input('email') }}" }) // Pass necessary data
                    })
